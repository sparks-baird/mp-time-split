...
# and `pip install mp-time-split` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "mp-time-split", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n mp-time-split -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "mp-time-split", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "mp-time-split", "YOUR_CONSOLE_SCRIPT"]
